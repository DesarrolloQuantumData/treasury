...
import { updaInterfaces } from '../services/interfaceService'
...
interface ModalinterfacesProps {
  ...
  isActive: boolean
  getDataInterfaces: any
}



export const ModalEditInterfaces: React.FC<ModalinterfacesProps> = ({
  showModal,
  setShowModal,
  selectedInterface,
  isActive,
  ...
  const [description, setDescription] = useState(selectedInterface.description)
  const [numberLinesHeader, setNumberLinesHeader] = useState(selectedInterface.numberLinesHeader)
  const [numberFinalLines, setNumberFinalLines] = useState(selectedInterface.numberFinalLines)
  ...
  const onSuccess = async (values: InterfacesFormValues) => {
    try {
      if (selectedInterface !== null) {
        await updaInterfaces(selectedInterface.id, { ...values, isActive })
      }
      onClose()
    } catch (error) {
      console.log(error)
      ...
      <form onSubmit={handleSubmit(onSuccess)}>
        <div className='space-y-3'>
          ...
          <div className='relative'>
            <InputField
              className='bg-transparent text-lg'
              defaultValue={selectedInterface?.numberFinalLines}
