...
export const useAccountsSelects = (Id?:number): UseAccountsSelectsReturn => {
  ...
  useEffect(() => {
    Promise.all([getBanksName(Id), getInternalConcept()])
      .then(([bankName, internalConcept]) => {
        setSelectOptionsFromApi('bankName', bankName)
        setSelectOptionsFromApi('internalConcept', internalConcept)
      })
      .catch(() => {
