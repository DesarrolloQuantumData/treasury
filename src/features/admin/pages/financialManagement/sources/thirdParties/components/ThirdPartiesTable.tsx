...
import { type Datum, getThirdParties, updateThirdParties } from '../services/thirdPartiesService'
import { ModalEditThirdPartiesForm } from './ModalEditThirdPartiesForm'

const columnHelper = createColumnHelper<Datum>()

export const ThirdPartiesTable: React.FC = () => {
  ...
  const columns = [
      ...
      cell: ({ row: { original } }) => (
        <div className='flex justify-center items-center'>
          <SwitchField
            ...
            onChange={async () => {
              await updateThirdParties(original.id!, { isActive: !original.isActive! })
              await getTableThirdParties(currentPage, 10)
            }}
          />
        </div>
      )
    ...
    columnHelper.accessor('Actions', {
      ...
      cell: ({ row }) => {
        ...
        const [selectedThirdParties, setSelectedThirdParties] = useState<Datum | null>(null)
          ...
          <>
            <Button
              ...
              onClick={() => {
                ...
                setSelectedThirdParties(row.original)
              }}
            >
  ...
  const getTableThirdParties = async (pageIndex: number, pageSize: number) => {
  ...
  const GetData = () => {
    getTableThirdParties(currentPage, 10)
  }

  useEffect(() => {
    getTableThirdParties(1, 10)
  }, [])
          ...
          <Button
            ...
            onClick={() => {
              if (currentPage > 1) {
                ...
                getTableThirdParties(currentPage - 1, 10)
              }
            }}
          >
