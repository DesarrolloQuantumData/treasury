...
import { ModalCreateThirdPartiesForm } from './components/ModalCreateThirdPartiesForm'
import { ThirdPartiesTable } from './components/ThirdPartiesTable'
import { getExcelThirdParties } from './services/thirdPartiesService'

export const ThirdPartiesPage: React.FC = () => {
  const [showModal, setShowModal] = useState(false)
  const downloadExcelFile = async () => {
    try {
      const response = await getExcelThirdParties()
  ...
  return (
    <div className='m-20 bg-primaryBackground-500 p-5 border-2 rounded-xl'>
      ...
      <ThirdPartiesTable />
      <ModalCreateThirdPartiesForm setShowModal={setShowModal} showModal={showModal} />
